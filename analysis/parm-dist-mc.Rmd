...
tmax <- 180
baseline5day <- run_scenario(tmax, list(T0=5), '5-day baseline')
baseline7p4day <- run_scenario(tmax, list(T0=7.4), '7.4-day baseline')
```
...
mc7p4 <- run_mc(N, distparms, tmax, list(T0=7.4), '7.4-day MC')
mc5 <- run_mc(N, distparms, tmax, list(T0=5), '5-day MC')
```
  ...
  tmp <-
    lapply(names(distparms), 
           function(parm) {
             dp <- distparms[parm]
             mc <- 
               run_mc(N, dp, tmax, list(T0=T0), scenName)
