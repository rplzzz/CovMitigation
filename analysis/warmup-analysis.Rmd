...
```{r loaddata}
datadir <- here('analysis', 'mcmc-rslts', 'mcmc-test-v3a')
...
runparms <- function(parms, scenname) {
  modparms <- parms[c('eta','xi','zeta', 'A0', 'D0', 'Ts', 'I0', 'mask_effect', 'mobility_scenario')]
  day_zero <- 30
...
## XXX diagnostic
mapparms['zeta'] <- 2
map_run <- runparms(as.list(mapparms), 'Frozen future')
## XXX end diagnostic
